...
class JointNERREPipeline:
    ...
    def train(self, n_epochs=150, checkpoint_path=None, save_freq=1, model_name="bert"):
        ...
        for epoch in range(start_epoch, start_epoch + n_epochs):
            ...
            for words, ner_labels, rel_labels in zip(
                ...
                rel_loss = torch.tensor(0.0).cuda()
                if len(rel_logits) > 0:
                    rel_loss = loss_fct(rel_logits, rel_labels).sum()  # Don't normalize yet

                out_rel = {}
                pred_idx = 0
                for i, e1 in enumerate(entity_spans):
                    for j, e2 in enumerate(entity_spans[i+1:], i+1):
                        if pred_idx < len(rel_logits):
    ...
    def save_checkpoint(self, epoch, metrics, model_name="bert", checkpoint_dir=None):
    ...
    def load_checkpoint(self, checkpoint_path=None, model_name="bert"):
    ...
    def save_epoch_results(self, epoch, metrics, model_name="bert", is_training=True):
        """
    ...
    def save_predictions(self, epoch, predicted_labels, predicted_relations, model_name="bert"):
        """
